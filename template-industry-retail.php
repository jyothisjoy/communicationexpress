            ...
            <section id="tab-section">
              <div class="container">
                ...
                <div id="components" class="tab-content active">
                  ...
                  <?php for ($a = 4; $a <= 5; $a++) {
                    $componentitems = get_field("componentitems_" . $a);
                    if (isset($componentitems) && $componentitems["com_title"] != "") {
                  ...
                  <div class="row justify-content-start my-5 d-in-flex">
                    <?php
                    $middlesecondGrid = get_field("middle_block_second_section");
                    if (isset($middlesecondGrid)) {
                      for ($v = 1; $v <= count($middlesecondGrid); $v++) {
                        $middlesecondGridcontent = $middlesecondGrid['content_' . $v];
                        if (($middlesecondGridcontent['title'] != "")) {
                          // echo comm_express_kses($middleGridcontent);
                          ?>
                          <div class="col-md-6  my-3">
                            <div class="twocolumngrid">

                              <?php if (!empty($middlesecondGridcontent['icon_image'])) { ?>
                                <img class="twocolumnimg"
                                  src="<?php echo esc_url(empty($middlesecondGridcontent['icon_image']) ? "" : $middlesecondGridcontent['icon_image']['url']); ?>" />
                              <?php } ?>
                              <div class="twocolumnheader mt-3">
                                <?php echo comm_express_kses($middlesecondGridcontent['title']); ?>
                              </div>
                              <div class="twocolumndesc">
                                <?php echo comm_express_kses($middlesecondGridcontent['description']); ?>
                              </div>

                              <a class="twocolumnurl"
                                href="<?php echo esc_url(empty($middlesecondGridcontent['read_more_link']) ? "" : $middlesecondGridcontent['read_more_link']['url']); ?>">Know
                                More</a>
                  ...
                  <div class="row justify-content-start my-5 d-in-flex">
                    <?php
                    $middlethirdGrid = get_field("middle_block_third_section");
                    if (isset($middlethirdGrid)) {
                      for ($v = 1; $v <= count($middlethirdGrid); $v++) {
                        $middlethirdGridcontent = $middlethirdGrid['content_' . $v];
                        if (($middlethirdGridcontent['title'] != "")) {
                          // echo comm_express_kses($middleGridcontent);
                          ?>
                          <div class="col-md-6  my-3">
                            <div class="twocolumngrid">

                              <?php if (!empty($middlethirdGridcontent['icon_image'])) { ?>
                                <img class="twocolumnimg"
                                  src="<?php echo esc_url(empty($middlethirdGridcontent['icon_image']) ? "" : $middlethirdGridcontent['icon_image']['url']); ?>" />
                              <?php } ?>
                              <div class="twocolumnheader mt-3">
                                <?php echo comm_express_kses($middlethirdGridcontent['title']); ?>
                              </div>
                              <div class="twocolumndesc">
                                <?php echo comm_express_kses($middlethirdGridcontent['description']); ?>
                              </div>

                              <a class="twocolumnurl"
                                href="<?php echo esc_url(empty($middlethirdGridcontent['read_more_link']) ? "" : $middlethirdGridcontent['read_more_link']['url']); ?>">Know
                                More</a>
